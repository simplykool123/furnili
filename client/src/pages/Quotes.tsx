...
import { Plus, Search, FileText, Download, Edit, Trash2, Eye, Calculator } from "lucide-react";
              ...
              <TableBody>
                {Array.isArray(quotes) && quotes.map((item: any) => (
                  <TableRow key={item.quote.id}>
                    <TableCell className="font-medium">{item.quote.quoteNumber}</TableCell>
                    <TableCell>
                      <div>
                        <div className="font-medium">{item.client?.name}</div>
                        <div className="text-sm text-gray-500">{item.client?.city}</div>
                      </div>
                    </TableCell>
                    <TableCell>
                      {item.project ? (
                        <div>
                          ...
                          <div className="text-sm text-gray-500">{item.project.code}</div>
                        </div>
                      ) : (
                    ...
                    <TableCell>
                      <div>
                        ...
                        {item.quote.description && (
                          <div className="text-sm text-gray-500 truncate max-w-48">{item.quote.description}</div>
                        )}
                      </div>
                    </TableCell>
                    <TableCell className="font-medium">₹{item.quote.totalAmount.toLocaleString()}</TableCell>
                    <TableCell>
                      <Badge variant={getStatusVariant(item.quote.status)}>
                        {item.quote.status}
                      </Badge>
                    </TableCell>
                    <TableCell>
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-3">
        ...
        <div className="space-y-3">
          ...
          {/* Sales Products Quick Add */}
          <div className="space-y-2">
            <h4 className="text-sm font-medium">Quick Add from Sales Products:</h4>
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel className="text-xs">Description</FormLabel>
              <FormControl>
                <Textarea {...field} placeholder="Item description" className="min-h-[60px]" />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
