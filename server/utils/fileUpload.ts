...
export const pettyCashMultipleImageUpload = multer({
  ...
  fileFilter: (req, file, cb) => {
    console.log('Petty cash multiple image filter - File details:', {
      originalname: file.originalname,
      mimetype: file.mimetype,
      fieldname: file.fieldname
    });
    ...
    if (mimetype && (extname || !file.originalname)) {
      console.log('Petty cash image accepted by multer filter');
      return cb(null, true);
    } else {
      console.log('Petty cash image rejected by multer filter - Invalid type');
